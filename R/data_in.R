...
#' @param name_in Nome della variabile nel data-frame di input.
...
#' @export
add_categorical_variable_from_raw_input <- function (name_in = '', variable_description = '', levels_in = c(), levels_out = c(), df_in = IN)
{
 ...
 OUT <- ordered(df_in[, c(name_in)], levels = levels_in)
        ...
        Hmisc::label(OUT) <- variable_description
 return(OUT)
}
