...
give_chisquare <- function (x = NA, y = NA, void_string = '-')
{
 ...
 out <- paste('\u03C7',
              ...
              ')',
              '=',
              give_nice(value = TEST$statistic, decimals = 2, text = '', with_equal_sign = FALSE, with_sign = FALSE, min_value = 0, max_value = Inf, void_string = '-'),
              note,
              ',',
              ' ',
...
give_fisher <- function (x = NA, y = NA, void_string = '-')
{
 ...
 if ((length(levels(XY$X))) == 2 & (length(levels(XY$Y) == 2)))
 {
  ...
  out <- paste('OR',
               '=',
               give_nice(value = abs(TEST$estimate), decimals = 3, text = '', with_equal_sign = FALSE, with_sign = FALSE, min_value = 0, max_value = Inf, void_string = '-'),
               ' ',
               '(',
               give_nice(value = abs(TEST$conf.int[1]), decimals = 2, text = '', with_equal_sign = FALSE, with_sign = FALSE, min_value = 0, max_value = Inf, void_string = '-'),
               ' , ',
               give_nice(value = abs(TEST$conf.int[2]), decimals = 2, text = '', with_equal_sign = FALSE, with_sign = FALSE, min_value = 0, max_value = Inf, void_string = '-'),
               ')',
               ',',
               ' ',
               give_nice_p(value = TEST$p.value, decimals = 3, with_p = TRUE, with_equal_sign = FALSE, with_stars = TRUE, multiple_stars = TRUE, alpha = 0.050, multiple_alphas = c(0.050, 0.010, 0.001), give_only_stars = FALSE, void_string = '-'),
